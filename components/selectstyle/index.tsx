  ...
  return (
    <div className="w-full">
      ...
      <div className="relative w-full aspect-square mb-6 bg-gray-100 rounded-lg overflow-hidden">
        {resultImage ? (
          <>
            ...
            <button
              ...
              className="absolute top-4 right-4 bg-white/90 hover:bg-white text-gray-800 px-4 py-2 rounded-full shadow-lg transition-all duration-200 flex items-center gap-2"
            >
        ...
        {/* 上传新图片按钮 - 当有图片时显示 */}
        {(uploadedImageUrl || resultImage) && (
          <button
            onClick={() => fileInputRef.current?.click()}
            className="absolute top-4 left-4 bg-white/90 hover:bg-white text-gray-800 px-4 py-2 rounded-full shadow-lg transition-all duration-200 flex items-center gap-2"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
